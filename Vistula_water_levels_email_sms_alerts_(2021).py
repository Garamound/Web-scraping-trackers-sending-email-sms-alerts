...
while True:
    ...
    for item in flow_values: flow_values_list.append(float(item))
    flow_values.reverse()
    while len(flow_values_list) < len(state_values_time_list) : flow_values_list.append(flow_values_list[-1])
    if (len(set(flow_values_list)) <= 1): flow_values_list = [0 for value in flow_values_list]
    #while len(flow_values_list) < len(state_values_time_list): flow_values_list.append(0)
    ...
    plt.title("Poziom Wisły w Bielanach: " + str(current_state_value) + "cm.        Czas pomiaru: " + current_state_time[0])
    plt.xlabel('Time')
    ...
    alarm_treshold = 1.1
    alarm_hours = 10
    alarm_rate = round((current_state_value / (sum(state_values_level_list[(len(state_values_level_list) - alarm_hours) :len(state_values_level_list)]) / alarm_hours)), 6)
    ...
    if alarm_rate < alarm_treshold: print("the alarm has not been activated. Alarm rate: ", alarm_rate)
    else:
        print("Level is increasing, alarm has been activated. Alarm rate: ", alarm_rate)
